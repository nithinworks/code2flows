...
// Dynamically import ArchitectureViewer with loading fallback
...
export default function Architecture() {
  ...
  const exampleDescription = `Our e-commerce platform uses a microservices architecture with the following components:

Frontend: React.js SPA with Next.js for SSR, hosted on Vercel
Backend Services:
- User Service (Node.js/Express) - handles authentication and user management
- Product Service (Python/FastAPI) - manages product catalog and inventory
- Order Service (Node.js/Express) - processes orders and payments
- Notification Service (Python/Flask) - handles emails and push notifications
...
- Route53 for DNS management`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoading(true);
      setError("");
      setLoadingStep("Analyzing description...");

      const {
        data: { session },
      } = await supabase.auth.getSession();

      const response = await fetch("/api/architecture-process", {
        method: "POST",
        headers: {
      ...
      setError(
        error instanceof Error ? error.message : "Failed to process description"
      );
      return;
    } finally {
      setLoading(false);
          ...
          <div className="container mx-auto px-4 py-8 md:py-16 flex flex-col items-center">
            ...
            <div className="text-center max-w-3xl mx-auto mb-8 md:mb-12 animate-fade-in stagger-1">
              ...
              <p className="text-lg md:text-xl text-[#001e2b]/60 font-medium">
                Transform your project description into a comprehensive
              ...
              <div className="bg-white p-4 md:p-8 rounded-lg border-2 border-[#001e2b] shadow-sm">
                ...
                >
                  <div className="space-y-4">
                    <div className="relative">
                      <textarea
                        ...
                        className="w-full h-64 p-4 pb-8 bg-white text-[#001e2b] rounded-md border-2 border-[#001e2b]/20 focus:border-[#00ed64] focus:ring-[#00ed64] transition placeholder-[#001e2b]/30 font-mono text-sm"
                        value={description}
                        onChange={(e) => setDescription(e.target.value)}
                      />
                      <div className="absolute bottom-2 right-3 text-xs text-[#001e2b]/40">
                        {wordCount}/{wordLimit} words
                      </div>
                    </div>

                    <div className="flex justify-between items-start text-sm text-[#001e2b]/60">
                      <div className="space-y-2">
                        <p className="font-medium">
                          Include in your description:
                        </p>
                        <ul className="list-disc list-inside space-y-1 text-[13px]">
                          <li>Core system components and services</li>
                      ...
                      <button
                        ...
                        className="text-[#0e0e0e] hover:text-[#00ee99]/80 transition text-sm font-medium"
                      >
                        See Example
                      </button>
                    </div>
                  ...
                  <button
                    type="submit"
                    disabled={
                      loading || !description.trim() || wordCount > wordLimit
                    }
